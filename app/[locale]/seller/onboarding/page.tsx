...
import { CheckCircle, ArrowRight, ArrowLeft, Shield, Upload, User, MapPin } from 'lucide-react';
import { toast } from 'sonner';

const IRAN_PROVINCES = [
  'تهران', 'اصفهان', 'فارس', 'خراسان رضوی', 'خوزستان', 'مازندران', 'کرمان', 'آذربایجان شرقی',
  'آذربایجان غربی', 'گیلان', 'کردستان', 'همدان', 'قزوین', 'زنجان', 'لرستان', 'البرز',
  'کرمانشاه', 'گلستان', 'یزد', 'سمنان', 'قم', 'مرکزی', 'هرمزگان', 'چهارمحال و بختیاری',
  'کهگیلویه و بویراحمد', 'بوشهر', 'ایلام', 'سیستان و بلوچستان', 'خراسان شمالی', 'خراسان جنوبی', 'اردبیل'
];
...
const verificationSchema = z.object({
  nationalId: z.string().min(8, 'National ID is required').max(20, 'National ID too long'),
...
export default function SellerOnboardingPage() {
  ...
  const steps = [
    { number: 1, title: t('profileInfo'), icon: User, completed: currentStep > 1 },
    { number: 2, title: t('contactInfo'), icon: MapPin, completed: currentStep > 2 },
          ...
          {currentStep === 1 && (
            <form onSubmit={handleProfileSubmit(onProfileSubmit)} className="space-y-6">
              <div className="text-center mb-6">
                ...
                <p className="text-muted-foreground">{t('profileInfoDescription')}</p>
              </div>
              ...
              <div>
                ...
                {profileErrors.shopName && (
                  <p className="text-sm text-destructive mt-1">{profileErrors.shopName.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {profileErrors.displayName && (
                  <p className="text-sm text-destructive mt-1">{profileErrors.displayName.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {profileErrors.bio && (
                  <p className="text-sm text-destructive mt-1">{profileErrors.bio.message}</p>
                )}
              </div>
          ...
          {currentStep === 2 && (
            <form onSubmit={handleContactSubmit(onContactSubmit)} className="space-y-6">
              <div className="text-center mb-6">
                ...
                <p className="text-muted-foreground">{t('contactInfoDescription')}</p>
              </div>
              ...
              <div>
                ...
                />
                {contactErrors.phone && (
                  <p className="text-sm text-destructive mt-1">{contactErrors.phone.message}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select onValueChange={(value) => setContactValue('province', value)}>
                    <SelectTrigger>
                  ...
                  {contactErrors.province && (
                    <p className="text-sm text-destructive mt-1">{contactErrors.province.message}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  />
                  {contactErrors.city && (
                    <p className="text-sm text-destructive mt-1">{contactErrors.city.message}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                {contactErrors.address && (
                  <p className="text-sm text-destructive mt-1">{contactErrors.address.message}</p>
                )}
              </div>
            ...
            <form onSubmit={handleVerificationSubmit(onVerificationSubmit)} className="space-y-6">
              ...
              <div>
                ...
                {verificationErrors.nationalId && (
                  <p className="text-sm text-destructive mt-1">{verificationErrors.nationalId.message}</p>
                )}
              ...
              <div>
                ...
                {uploadedDocs.length > 0 && (
                  <div className="mt-4">
                    <p className="text-sm font-medium mb-2">{t('uploadedDocuments')}:</p>
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="agreeToTerms" className="text-sm">
                  {t('agreeToTerms')} <a href="/legal/terms" className="text-primary underline">{t('termsAndConditions')}</a>
                </Label>
              </div>
              {verificationErrors.agreeToTerms && (
                <p className="text-sm text-destructive">{verificationErrors.agreeToTerms.message}</p>
              )}
