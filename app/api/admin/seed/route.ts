...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isDevelopment && !isTest && process.env.ENABLE_SEED_ENDPOINT !== 'true') {
    ...
    if (!isDevelopment) {
      if (!validToken || validToken.length < 32) {
        console.error('SEED_TOKEN must be at least 32 characters in production');
      ...
      Buffer.from(providedToken).copy(providedBuffer, 0, 0, Math.min(providedToken.length, fixedLength));
      Buffer.from(validToken).copy(expectedBuffer, 0, 0, Math.min(validToken.length, fixedLength));
