...
import { sendEmail, generateResetToken, getResetUrl, isValidEmail, checkEmailRateLimit } from '@/lib/email';
import { z } from 'zod';
...
export const POST = withRateLimit(authRateLimit, withCSRF(async function(request: NextRequest) {
  try {
    ...
    if (user) {
      try {
        ...
        const emailResult = await sendEmail({
          to: user.email,
          subject: locale === 'fa' ? 'بازیابی رمز عبور - کیارا کرافت' : 'Password Reset - Kiara Kraft',
        ...
        if (!emailResult.success) {
          ...
          console.log(`Password reset email sent to ${user.email} via ${emailResult.provider}`);
        }
    ...
    const successMessage = locale === 'fa' 
