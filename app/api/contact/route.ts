...
const contactRateLimit = createRateLimiter({ windowMs: 10 * 60 * 1000, maxRequests: 5 });

export const POST = withRateLimit(contactRateLimit, async (req: NextRequest) => {
  try {
    ...
    const result = await sendEmail({ to, subject, html, text: `${name} <${email}>\n\n${message}` });

    if (!result.success) {
      return new Response(JSON.stringify({ error: result.error || 'Failed to send message' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
      });
    }
