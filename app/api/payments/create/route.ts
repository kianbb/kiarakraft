  ...
  withCSRF(async function (request: NextRequest) {
    try {
      ...
      const items = await prisma.orderItem.findMany({
        ...
        include: {
          product: {
            select: { id: true, title: true, stock: true, active: true },
          },
        },
      });
      ...
      const insufficient = collectPreflightIssues(
        items.map(it => ({
          ...
          product: {
            ...
            active: it.product?.active ?? false,
          },
        }))
      );
      ...
      if (envBase) {
        try {
          ...
          const allowedHosts = (
            process.env.ALLOWED_APP_BASE_HOSTS || requestHost
          )
            .split(',')
            .map(h => h.trim())
            .filter(Boolean);
