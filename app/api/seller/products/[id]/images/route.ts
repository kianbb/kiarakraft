...
export const POST = withRateLimit(uploadRateLimit, withCSRF(async function(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const existingCount = await prisma.listingImage.count({ where: { productId } });
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        return NextResponse.json({ error: `File too large: max ${Math.round(MAX_FILE_SIZE / (1024*1024))}MB` }, { status: 400 });
      }

      const buffer = Buffer.from(await file.arrayBuffer());
      const uploaded = await uploadImageToCloudinary(buffer, { folder: `kiarakraft/products/${productId}` });
      const created = await prisma.listingImage.create({ data: { productId, url: uploaded.secure_url, sortOrder: nextSort++ } });
      results.push({ id: created.id, url: created.url, sortOrder: created.sortOrder });
    }
...
export const PATCH = withRateLimit(uploadRateLimit, withCSRF(async function(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updates: Array<{ id: string; sortOrder: number }> = body?.order ?? [];
    if (!Array.isArray(updates) || updates.length === 0) {
      return NextResponse.json({ error: 'No order provided' }, { status: 400 });
    }

    await Promise.all(
      updates.map((u) => prisma.listingImage.update({ where: { id: u.id }, data: { sortOrder: u.sortOrder } }))
    );
