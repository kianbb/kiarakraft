...
export function chooseLocale(locale?: string): Locale {
  ...
  return baseLocale && locales.includes(baseLocale as Locale)
...
export default getRequestConfig(({ locale }) => {
  ...
  if (!locale) {
    console.warn(
      '[i18n/request.ts] No locale provided - this should not happen with proper middleware'
    );
    // For now, fall back to Persian (default) but this should be investigated
    const chosen = 'fa';
    console.log(
      '[i18n/request.ts] Chosen locale (emergency fallback):',
      chosen
    );
    return {
      locale: chosen,
      ...
      onError: error => {
        console.error(
          '[i18n/request.ts] Translation error for locale',
          chosen,
          ':',
          error
        );
      ...
      getMessageFallback: ({ namespace, key }) => {
        console.warn(
          '[i18n/request.ts] Missing translation key:',
          `${namespace}.${key}`,
          'for locale:',
          chosen
        );
