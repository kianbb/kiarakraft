...
export type MinimalTx = {
  order: { update(args: { where: { id: string }; data: { status: 'CANCELED' } }): Promise<unknown> };
  cart: { upsert(args: { where: { userId: string }; create: { userId: string }; update: Record<string, never> }): Promise<{ id: string }> };
  cartItem: { upsert(args: { where: { cartId_productId: { cartId: string; productId: string } }; update: { quantity: number }; create: { cartId: string; productId: string; quantity: number } }): Promise<unknown> };
};
...
export async function cancelOrderAndRestoreCart(
  ...
  const cart = await tx.cart.upsert({ where: { userId }, create: { userId }, update: {} as Record<string, never> });
