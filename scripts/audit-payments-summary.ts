...
async function main() {
  ...
  console.log(`✅ Test Product: ${testProduct?.title} (Current Stock: ${testProduct?.stock})`);
  ...
  for (const order of orders) {
    ...
    if (order.payment) {
      console.log(`   ├─ Payment: ${order.payment.status} (${order.payment.id.slice(-8)})`);
  ...
  console.log(`   ✅ Stock Atomicity: ${testProduct?.stock === (10 - stockDecremented) ? 'PASS' : 'FAIL'}`);
