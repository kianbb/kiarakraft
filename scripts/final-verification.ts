...
class ProductionVerifier {
  ...
  async verifyTaskA(): Promise<void> {
    ...
    try {
      ...
      const withoutComments = loadingContent
        .replace(/\/\*[\s\S]*?\*\//g, '')
        .replace(/\/\/.*$/gm, '');
      const hasLoadingText =
        withoutComments.includes('"Loading..."') ||
        withoutComments.includes("'Loading...'");

      if (hasLoadingText) {
        this.log(
          'fail',
          'A1: Loading text removal',
          'Still contains "Loading..." text strings'
        );
      } else {
        this.log(
          'pass',
          'A1: Loading text removal',
          'No "Loading..." text in server HTML'
        );
      }
    } catch (error) {
    ...
    try {
      const response = await fetch(
        'http://localhost:3000/fa/product/handmade-ceramic-bowl'
      );
      ...
      if (hasMetadata) {
        this.log(
          'pass',
          'A2: Product SSR metadata',
          'Product pages render complete metadata'
        );
      } else {
        this.log(
          'fail',
          'A2: Product SSR metadata',
          'Missing proper metadata rendering'
        );
      }
    } catch (error) {
  ...
  async verifyTaskB(): Promise<void> {
    ...
    for (const slug of validSlugs) {
      try {
        const response = await fetch(
          `http://localhost:3000/fa/product/${slug}`
        );
    ...
    if (validCount === validSlugs.length) {
      this.log(
        'pass',
        'B1: Valid product slugs',
        `${validCount}/${validSlugs.length} valid slugs working`
      );
    } else {
      this.log(
        'fail',
        'B1: Valid product slugs',
        `Only ${validCount}/${validSlugs.length} valid slugs working`
      );
    }
    ...
    try {
      const response = await fetch(
        'http://localhost:3000/fa/product/nonexistent-product-123'
      );
      const html = await response.text();
      const hasNotFound =
        html.includes('NEXT_NOT_FOUND') || html.includes('محصول یافت نشد');

      if (hasNotFound) {
        this.log(
          'pass',
          'B2: Invalid slug 404s',
        ...
        this.log(
          'fail',
          'B2: Invalid slug 404s',
          'Invalid slugs not returning proper 404'
        );
      }
    } catch (error) {
  ...
  async verifyTaskC(): Promise<void> {
    ...
    try {
      ...
      this.log(
        'pass',
        'C1: Payment system',
        `${gateway} gateway, ${paymentCount} payments, ${orderCount} orders`
      );
    } catch (error) {
    ...
    try {
      ...
      if (cloudName && apiKey) {
        ...
        this.log(
          'pass',
          'C2: Cloudinary uploads',
          `${assets.length} assets in cloud storage`
        );
      } else {
    ...
    try {
      ...
      this.log(
        'pass',
        'C3: Search system',
        `Persian search returns ${results.products.length} results`
      );
    } catch (error) {
    ...
    try {
      ...
      // C4: Email system
      const emailRouteExists = fs.existsSync(
        'app/api/auth/reset-password/route.ts'
      );
      this.log(
        emailRouteExists ? 'pass' : 'warn',
        'C4: Email flows',
        emailRouteExists ? 'Reset password API exists' : 'Email API missing'
      );
      ...
      this.log(
        hasAnalytics ? 'pass' : 'warn',
        'C5: Analytics & monitoring',
        hasAnalytics ? 'Analytics configured' : 'No analytics configured'
      );

      // C6: PWA
      const manifestExists = fs.existsSync('public/manifest.webmanifest');
      this.log(
        manifestExists ? 'pass' : 'warn',
        'C6: PWA support',
        manifestExists ? 'PWA manifest exists' : 'PWA not configured'
      );
      ...
      this.log(
        ...
        'C7: Trust & safety',
        securityOK ? 'CSRF + Rate limiting active' : 'Security measures missing'
      );
    } catch (error) {
  ...
  async verifyDatabase(): Promise<void> {
    ...
    try {
      ...
      this.log(
        'pass',
        'Database connection',
        `${users} users, ${products} products, ${orders} orders, ${categories} categories`
      );
      ...
      this.log(
        hasSearchExtensions ? 'pass' : 'fail',
        'Search extensions',
        `${extensions.length}/2 PostgreSQL search extensions`
      );
    } catch (error) {
  ...
  async verifyEnvironment(): Promise<void> {
    ...
    const requiredEnvs = ['DATABASE_URL', 'NEXTAUTH_SECRET', 'PAYMENT_GATEWAY'];
    ...
    this.log(
      'pass',
      'Optional configs',
      `${optionalCount}/${optionalEnvs.length} optional configs set`
    );
  }
  ...
  printSummary(): void {
    ...
    if (failed === 0) {
      ...
      console.log(
        `${RED}🚨 Address failures before production deployment${RESET}\n`
      );
