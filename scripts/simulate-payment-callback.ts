...
async function simulateSuccessfulPayment(
  ...
  for (const update of result.stockUpdates) {
    console.log(
      `   Product ${update.productId}: -${update.quantity} = ${update.newStock} remaining`
    );
  }
...
async function main() {
  ...
  if (args.length < 3) {
    console.log(
      'Usage: npx tsx simulate-payment-callback.ts <orderId> <paymentId> <authority> [success|failed]'
    );
    console.log('\nExample:');
    console.log(
      'npx tsx simulate-payment-callback.ts cmejw87t30001oc89jd5bjhom cmejw88u20005oc8906ekfyph audit-payment-1755689552569 success'
    );
    return;
  }
  ...
  for (const item of initialOrder.items) {
    console.log(
      `   Product stock: ${item.product.stock} (${item.product.title})`
    );
  }
  console.log('');
