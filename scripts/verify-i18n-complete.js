...
function verifyTranslationFiles() {
  ...
  console.log(
    `   ✅ English translations loaded (${Object.keys(en).length} top-level keys)`
  );
  console.log(
    `   ✅ Persian translations loaded (${Object.keys(fa).length} top-level keys)`
  );
...
function verifyKeyCompleteness(en, fa) {
  ...
  if (missingInFA.length === 0 && missingInEN.length === 0) {
    console.log(
      `   ✅ Perfect key coverage! Both locales have ${enKeys.length} keys`
    );
  }
...
async function testServerEndpoints() {
  ...
  const testCases = [
    {
      path: '/en',
      expected: 'English',
      markers: ['Featured Categories', 'lang="en"', 'dir="ltr"'],
    },
    {
      path: '/fa',
      expected: 'Persian',
      markers: ['دسته‌بندی‌های ویژه', 'lang="fa"', 'dir="rtl"'],
    },
  ];
  ...
  try {
    ...
    try {
      ...
      console.log(
        '   ⚠️  No development server detected. Start with `npm run dev` to test endpoints.'
      );
  ...
  for (const testCase of testCases) {
    try {
      const { stdout } = await execAsync(
        `curl -s "${serverUrl}${testCase.path}"`
      );

      const foundMarkers = testCase.markers.filter(marker =>
        stdout.includes(marker)
      );
      const success = foundMarkers.length === testCase.markers.length;

      console.log(
        `   ${success ? '✅' : '❌'} ${testCase.expected} locale (${testCase.path})`
      );

      if (!success) {
        console.log(
          `      Missing markers: ${testCase.markers.filter(m => !foundMarkers.includes(m)).join(', ')}`
        );
      } else {
        console.log(
          `      Found all expected markers: ${testCase.markers.join(', ')}`
        );
      }
...
function checkPotentialIssues() {
  ...
  console.log(
    '   ℹ️  This is a basic check. Manual review is recommended for thorough verification.'
  );

  // Check if common Persian/Arabic characters exist in component files
  try {
    exec(
      'grep -r "[\u0600-\u06FF]" components/ app/ --exclude-dir=node_modules 2>/dev/null | head -5',
      (error, stdout) => {
        if (stdout.trim()) {
          ...
          console.log(
            '   💡 Verify these are in translation files, not hardcoded'
          );
        } else {
          console.log(
            '   ✅ No obvious hardcoded Persian/Arabic text found in components'
          );
        }
      }
    );
    ...
    console.log(
      '   ℹ️  Could not check for hardcoded strings (grep not available)'
    );
  }
}
...
async function runVerification() {
  try {
    ...
    console.log(
      `Translation key coverage: ${keyCheck.complete ? '✅ PERFECT' : '⚠️  NEEDS ATTENTION'}`
    );
    ...
    if (serverCheck.serverRunning) {
      ...
      console.log(
        `Server endpoints: ${allEndpointsWorking ? '✅ ALL WORKING' : '❌ SOME ISSUES'}`
      );
    } else {
    ...
    if (
      keyCheck.complete &&
      (!serverCheck.serverRunning || serverCheck.results.every(r => r.success))
    ) {
